...
////        add_action( 'delete_user', array('STM_LMS_User::remove_user'), 10 );
	...
	$uEmail = $data['user_login'];

        if($uEmail){
            global $wpdb;
            $cusUQ = "SELECT u.user_status FROM sb_users u WHERE u.user_email = '$uEmail' LIMIT 1;";
                
            $cusU = $wpdb->get_results( $cusUQ, ARRAY_A )[0];

            if($cusU['user_status']){
                $r['message'] = esc_html__('User has been blocked due to Non Payment,please contact your administrator', 'masterstudy-lms-learning-management-system');
                wp_send_json($r);
                die;
            }
        }
