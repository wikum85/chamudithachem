...
if ( is_user_logged_in() ) {
    ...
    global $wpdb;
    $p_contQ = "SELECT p.post_content
                FROM sb_user_record ur
                INNER JOIN sb_posts p ON p.id = ur.course_id
                WHERE ur.user_id = $user->ID
                LIMIT 1;";

    $p_cont = $wpdb->get_results( $p_contQ, ARRAY_A )[0]; 

    $shortcode = 'stm_zoom_conference';
    $pattern = get_shortcode_regex();

    if ( preg_match_all( '/'. $pattern .'/s', $p_cont['post_content'], $matches ) && array_key_exists( 2, $matches ) && in_array( $shortcode, $matches[2] ) )  {
        $shortcode_atts = array_keys($matches[2], $shortcode);
        
        if (!empty($shortcode_atts)) {
            foreach($shortcode_atts as $att) {
                preg_match('/post_id="(\d+)"/', $matches[3][$att], $post_ID);
                $post_IDs[] = $post_ID[1];
            }
            if(!in_array($post_id, $post_IDs) || count($post_IDs) == 0){
                noPermission();
            }
        }else{
            noPermission();
